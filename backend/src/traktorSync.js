...
export async function syncTraktorNML() {
    console.log(`Starting Traktor NML Sync from ${PLAYLISTS_DIR}...`);
    
    if (!fs.existsSync(PLAYLISTS_DIR)) {
        console.error(`Playlists directory not found: ${PLAYLISTS_DIR}`);
        return;
    }

    const files = fs.readdirSync(PLAYLISTS_DIR).filter(f => f.endsWith('.nml'));
    
    if (files.length === 0) {
        console.log('No .nml files found to sync.');
        return;
    }
    ...
    for (const file of files) {
        ...
        const xmlData = fs.readFileSync(path.join(PLAYLISTS_DIR, file), 'utf8');
        ...
        console.log(`Found ${entries.length} tracks in ${file}.`);
