...
import { Clapperboard, Scissors, Film } from 'lucide-react';
...
export default function VideoHighlight() {
  ...
  const handleGenerate = () => {
    ...
    setTimeout(() => {
      setStatus("done");
      alert("Highlight generation sent to Blender rendering queue on the Mac!");
    }, 2000);
  };
    ...
    <div className="p-8 h-screen flex gap-8">
      <div className="w-96 flex flex-col gap-6">
        ...
        <div className="text-sm opacity-60 bg-slate-900 p-4 border border-slate-800 rounded-lg">
          <p><strong>Note:</strong> Video rendering happens natively on your Mac using Blender and ffmpeg scripts located in `plugins/video-highlight`.</p>
        </div>
      </div>
      ...
      <div className="flex-1 bg-slate-900 border border-slate-800 rounded-xl shadow-2xl flex flex-col items-center justify-center p-8 text-center gap-4">
        {status === "done" ? (
           <div className="text-primary flex flex-col items-center gap-4">
             ...
             <p className="text-slate-400">output_highlight.mp4 has been saved to your Assets folder.</p>
           </div>
        ) : (
          <>
